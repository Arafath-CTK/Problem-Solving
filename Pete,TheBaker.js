...
// simplicity there are no units for the amounts (e.g. 1 lb of flour or 200 g of sugar are simply 1 or 200).
...
function cakes(recipe, available) {
  ...
  const cakesPerIngredient = [];
  for (const ingredient in recipe) {
    const needed = recipe[ingredient];
    const availableAmount = available[ingredient] || 0;
    cakesPerIngredient.push(Math.floor(availableAmount / needed));
  }
