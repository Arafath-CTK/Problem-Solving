...
// takes you one minute to traverse one city block, so create a function that will return true if the walk the
...
function isValidWalk(walk) {
  if (walk.length === 10) {
    ...
    walk.forEach((element) => {
      directionCount[element] = (directionCount[element] || 0) + 1;
    });
    if (
